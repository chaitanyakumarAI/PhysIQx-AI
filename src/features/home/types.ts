...
/**
 * One coach-authored action for today ("Home becomes coach, not dashboard").
 * UI contract only — in the real system these are generated by the AI Coach
 * from score/fuel/mission state; the mock derives them from the same fixtures
 * so they can never contradict the numbers shown elsewhere on the screen.
 * `iconId` (not a component) so the list can cross the server→client boundary.
 */
